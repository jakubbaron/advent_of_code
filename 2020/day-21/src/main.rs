...
fn main() -> io::Result<()> {
    let files_results = vec![
        ...
        ("input.txt", 2485, "bqkndvb,zmb,bmrmhm,snhrpv,vflms,bqtvr,qzkjrtl,rkkrx"),
    ];
    for (f, result_1, result_2) in files_results.into_iter() {
        ...
        let mut allergens: HashMap<String, HashSet<String>> = HashMap::with_capacity(file_content.len());
        ...
        for line in file_content.into_iter() {
            ...
            for eng_word in english.iter() {
                allergens.entry(eng_word.to_string()).or_insert(elvish.clone());
        ...
        let final_list:Vec<String> = words.into_iter().map(|(_, val)| val.into_iter().next().unwrap()).collect();
